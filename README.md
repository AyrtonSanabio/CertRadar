...
O repositório contém um núcleo nativo em C++17, testes automatizados x64/Win32 e uma interface de desenvolvimento para busca segura de certificados A1 e consulta dos certificados instalados no usuário atual. Coleta, diagnóstico, ações autorizadas, reteste, perfil PJe e pacotes portáteis de laboratório já foram implementados. A qualificação em VMs, a prova com A3 real, a licença e a assinatura de código ainda bloqueiam uma release pública.
...
Já existe uma interface de desenvolvimento em `build\x64\Debug\certradar.exe`; ela mostra o ambiente detectado e somente inicia a busca após o usuário pressionar **Iniciar busca**. Ao terminar, **Resumo** disponibiliza o ambiente e contagens sanitizadas para o chamado, enquanto **Arquivo** abre a pasta com o candidato selecionado sem executar o PFX/P12. **Cert. usuário** e **Cert. máquina** leem, em segundo plano e sem alterar o sistema, os respectivos stores Pessoais, colocam situações que exigem atenção primeiro e mostram somente metadados mínimos para suporte. **A3 local** verifica serviço, quantidade de leitores e presença de cartão/token sem abrir sessão nem usar PIN. Em cada visualização, **Resumo** copia apenas fatos sanitizados do resultado ativo. Os binários dessa pasta ainda não constituem uma release assinada.
