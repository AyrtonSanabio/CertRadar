# Índice da documentação

Esta pasta é a fonte de verdade para o escopo e as decisões técnicas do CertRadar. A implementação futura deverá apontar para requisitos identificados nestes documentos.

## Ordem de leitura recomendada

1. [Visão do produto](01_VISAO_DO_PRODUTO.md)
2. [Escopo e requisitos](02_ESCOPO_E_REQUISITOS.md)
3. [Arquitetura proposta](03_ARQUITETURA.md)
4. [Segurança e privacidade](04_SEGURANCA_E_PRIVACIDADE.md)
5. [Estratégia de testes](05_ESTRATEGIA_DE_TESTES.md)
6. [Busca de certificados](06_BUSCA_DE_CERTIFICADOS.md)
7. [Fluxo de suporte](07_FLUXO_DE_SUPORTE.md)
8. [Roadmap](08_ROADMAP.md)
9. [Decisões para revisão](09_DECISOES_PARA_REVISAO.md)
10. [Fluxo de implementação acompanhada](10_FLUXO_DE_IMPLEMENTACAO_ACOMPANHADA.md)
11. [Contrato de fatos](11_CONTRATO_DE_FATOS.md)
12. [Perfil de aplicação PJe](12_PERFIL_PJE.md)
13. [Pacote portátil Windows 10/11](13_PACOTE_PORTATIL.md)
14. [Matriz de compatibilidade e evidências de VM](14_MATRIZ_COMPATIBILIDADE.md)
15. [Modo legado Windows XP SP3](15_MODO_LEGADO_XP.md)
16. [Hardening e análise estática](16_HARDENING.md)
17. [Auditoria de privacidade](17_AUDITORIA_DE_PRIVACIDADE.md)
18. [Gate de release candidate](18_RELEASE_CANDIDATE.md)
19. [Resumo sanitizado da busca](19_RESUMO_SANITIZADO_DA_BUSCA.md)
20. [Localização segura do arquivo](20_LOCALIZACAO_SEGURA_DO_ARQUIVO.md)
21. [Ambiente visível na interface](21_AMBIENTE_VISIVEL.md)
22. [Ambiente no resumo do chamado](22_AMBIENTE_NO_RESUMO.md)
23. [Certificados instalados na interface](23_CERTIFICADOS_INSTALADOS_NA_INTERFACE.md)
24. [Resumo sanitizado dos certificados instalados](24_RESUMO_CERTIFICADOS_INSTALADOS.md)
25. [Prioridade visual dos certificados instalados](25_PRIORIDADE_CERTIFICADOS_INSTALADOS.md)
26. [Store Pessoal da máquina na interface](26_STORE_DA_MAQUINA_NA_INTERFACE.md)
27. [Diagnóstico A3 local na interface](27_DIAGNOSTICO_A3_LOCAL.md)
28. [Orientação A3 por evidência local](28_ORIENTACAO_A3_LOCAL.md)
29. [Estado da implementação](STATUS.md)
30. [Glossário](GLOSSARIO.md)
31. [Referências oficiais](REFERENCIAS.md)

## Regra de alteração

Uma mudança que afete comportamento, segurança, sistemas suportados ou coleta de dados deverá atualizar primeiro a documentação relevante e seus critérios de aceite. A implementação e os testes deverão ser vinculados ao requisito correspondente.
